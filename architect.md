...
This allows Architect to extend perceptual bass response down to 1hz by exploiting a known phenomenon called the ***Missing Fundamental Effect***. Your hearing will infer the missing tones from the signal that Architect produces, particularly the harmonics generated by the interactions between the 30hz carrier frequency and the original tone. This technique is used in phone speakers, earbuds, and many bluetooth speakers. 

## Electromagnetic Force Braking System (EMFB)
A speaker driver is at its most simple level an electromagnetic device. It follows properties closely related to an electric motor. Thanks to my custom amplification architecture, I can use Architect to directly control the driver cones, by shorting their terminals to ground. This works in a similar manner to regenerative braking in an electric vehicle. It turns the driver's coil and magnet into a generator, using the amplifier itself to slow down the cone instead of letting it slow down on its own using friction. By doing this, we can introduce an instantaneous braking force on the cone, capable of violently cancelling its inertia. This allows for more precise control for the tweeters, allowing them to produce higher tones than they would otherwise be capable of (due to stronger phase-coherence - it stops the dual tweeters from cancelling each other out due to inconsistent phase shift at very high frequencies). Additionally, it improves the subwoofer driver's control of the passive radiator. This allows the speaker to partially or entirely cancel out the boominess and lack of tightness that usually accompanies a passive radiator based system, by exploiting the pneumatic coupling it shares to the subwoofer cone. 
...
This allows Architect to recalculate compression thresholds in real-time, adapt to driver aging, and prevent damage without the user noticing. These data are then used to protect the drivers, and maintain safe operation. Architect is directly able to monitor the excursion and relative cone position of the three drivers using a known mapping of digital signal level, frequency, and excursion. This mapping was produced via tens of hours of testing and logging.
